...
object ChatServer {
    ...
    fun sendMessage(message: String): Boolean {
        ...
        messageCharacteristic?.let { characteristic ->
            ...
            gatt?.let {
                ...
                log(TAG, "onServicesDiscovered: message send: $success")
                if (success) {
                    _messages.value = MessageModel.LocalMessage(message)
                }
            } ?: run {
                log(TAG, "sendMessage: no gatt connection to send a message with")
            }
        }
        return false
    }
    ...
    private fun startAdvertisement() {
        ...
        log(TAG, "startAdvertisement: with advertiser $advertiser")
    ...
    private fun stopAdvertising() {
        log(TAG, "Stopping Advertising with advertiser $advertiser")
    ...
    private class GattServerCallback : BluetoothGattServerCallback() {
        override fun onConnectionStateChange(device: BluetoothDevice, status: Int, newState: Int) {
            ...
            log(TAG, "onConnectionStateChange: Server $device ${device.name} success: $isSuccess connected: $isConnected")
        ...
        override fun onCharacteristicWriteRequest(
            ...
            if (characteristic.uuid == MESSAGE_UUID) {
                ...
                log(TAG, "onCharacteristicWriteRequest: Have message: \"$message\"")
                message?.let { _messages.postValue(MessageModel.RemoteMessage(it)) }
            }
        }
    }
    ...
    private class GattClientCallback : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            val isSuccess = status == BluetoothGatt.GATT_SUCCESS
            val isConnected = newState == BluetoothProfile.STATE_CONNECTED
            log(TAG, "onConnectionStateChange: Client $gatt  success: $isSuccess connected: $isConnected")
        ...
        override fun onServicesDiscovered(discoveredGatt: BluetoothGatt, status: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                log(TAG, "onServicesDiscovered: Have gatt $discoveredGatt")
                gatt = discoveredGatt
    ...
    private class DeviceAdvertiseCallback : AdvertiseCallback() {
        override fun onStartFailure(errorCode: Int) {
            ...
            log(TAG, "Advertising failed: $errorMessage")
            //_viewState.value = DeviceScanViewState.Error(errorMessage)
        }

        override fun onStartSuccess(settingsInEffect: AdvertiseSettings) {
            super.onStartSuccess(settingsInEffect)
            log(TAG, "Advertising successfully started")
        }
    }
}